...
//  iOS CAIO
//
//  Created by Venkata Prabhu on 03.02.2024.
//

import UIKit
/**
 `MyEducationViewCell` is a custom `UITableViewCell` designed to display a cell with an icon image and a title. It is specifically created for showcasing schools in a table view.

  ## Properties:
  - `cellView`: A UIView representing the main container for the cell's content.
  - `iconImage`: An UIImageView for displaying the education icon.
  - `cellTitle`: A UILabel for displaying the title of the education section.
  ...
  ## Example Usage:
  ```
  let cell = MyEducationViewCell(style: .default, reuseIdentifier: "EducationCell")
