...
//  iOS CAIO
//
//  Created by Caio Chaves on 14.01.2024.
//
...
class EducationViewController: UIViewController {
    ...
    private func configureLayout() {
        ...
        background.image = UIImage(named: "myEducation")
...
extension EducationViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            cell.school = MyEducationModel.education[dataIndex]

            cell.graduateInfo = MyEducationModel(rawValue: MyEducationModel.education[dataIndex])?.description
            cell.graduatedString = MyEducationModel(rawValue: MyEducationModel.education[dataIndex])?.graduate
            return cell
        }
    ...
    /*
     ...
     print(indexPath.row)
     let certificateCell = tableView.dequeueReusableCell(withIdentifier: EducationIdentifiers.certificate.rawValue, for: indexPath) as!
     CertificateViewCell
     ...
     certificateCell.certificate = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[0])?.rawValue
     certificateCell.school = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[0])?.school
     certificateCell.dateString = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[0])?.dates
     
     return certificateCell
     
     } else if indexPath.row == 6 {
     certificateCell.certificate = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[1])?.rawValue
     certificateCell.school = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[1])?.school
     certificateCell.dateString = MyCertificateModel(rawValue: MyCertificateModel.cetificateName[1])?.dates
